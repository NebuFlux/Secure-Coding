...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Find the start point of the user input data
  std::string::size_type queryStart = sql.find("NAME=");

  // if the query has input data
  if ( queryStart != sql.npos)
  {
      // account for exact start of user input data
      queryStart += 5;

      // If space is found in user input section of query
      if (sql.find(' ', queryStart) != sql.npos)
      {
          // print SQL Injection message and return
          std::cout << "\n\t***Suspected SQL Injection***" << std::endl;
          std::cout << "\tSubmitted Query: " << std::quoted(sql.substr(queryStart)) << std::endl;
          return false;
      }
  }
