// NumericOverflow.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
/// <throws>overflow error for exceeding maximum limit for type
/// <throws>underflow error for exceeding minimum limit for type
template <typename T>
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        ...
        if (increment < 0 && result < std::numeric_limits<T>::lowest() - increment)
        {
            ...
            uError << "\n\t***Adding " << +increment << " to " << +result << " exceeds " << typeid(T).name()
                << " lowest value of: " << +std::numeric_limits<T>::lowest() << "***";

            // Throw underflow exception
            throw std::underflow_error(uError.str());
        }
        // Check for overflow when adding a positive number
        else if (increment > 0 && result > std::numeric_limits<T>::max() - increment)
        {
...
/// <param name="steps">The number of steps to iterate</param>
/// <returns>start - (increment * steps)</returns>
/// <throws>underflow exception for exceeding minimum limit for type
/// <throws>overflow exception for exceeding maximum limit for type
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // check for overflow when subtracting a negative
        if (decrement < 0 && result > std::numeric_limits<T>::max() + decrement)
        {
            // Create overflow error string
            std::ostringstream oError;
            oError << "\n\t***Subtracting " << +decrement << " from " << +result << " exceeds " << typeid(T).name()
                << " max value of: " << +std::numeric_limits<T>::max() << "***";

            // Throw overflow exception
            throw std::overflow_error(oError.str());
        }
        // check for underflow when subtracting a postive
        else if (decrement > 0 && result < std::numeric_limits<T>::lowest() + decrement)
        {
            // Create underflow error string
            std::ostringstream uError;
            uError << "\n\t***Subtracting " << +decrement << " from " << +result << " exceeds " << typeid(T).name()
        ...
        // pass all checks complete subtraction
        result -= decrement;
    }
...
void test_underflow()
{
    ...
    try 
    {
        ...
        std::cout << "\tSubtracting Numbers With Underflow (" << +start << ", " << +decrement << ", " << (steps * 3) << ") = ";
        result = subtract_numbers<T>(start, decrement, steps * 3);
        std::cout << +result << std::endl;
    }
